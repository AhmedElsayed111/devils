...
vipdevils = Token:match("(%d+)")
redis:set(vipdevils..":VERSION",1)
redis:set(vipdevils..":SUDO_ID:",SUDO_USER)
redis:set(vipdevils..":DataCenter:",'German')
redis:set(vipdevils..":UserNameBot:",BOT_User)
redis:set(vipdevils..":NameBot:",BOT_NAME)
redis:hset(vipdevils..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3vipdevils_INSTALL","Yes")
info = {} 
...
vipdevils = Token:match("(%d+)")
our_id = tonumber(vipdevils)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(vipdevils..":UserNameBot:")
SUDO_ID = tonumber(redis:get(vipdevils..":SUDO_ID:"))
SUDO_USER = redis:hgetall(vipdevils..'username:'..SUDO_ID).username
version = redis:get(vipdevils..":VERSION")
DataCenter = redis:get(vipdevils..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..vipdevils..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(vipdevils..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(vipdevils..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(vipdevils..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(vipdevils..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور '
msg.TheRank = 'مطور البوت '
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(vipdevils..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(vipdevils..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء '
msg.TheRank = 'المنشىء '
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(vipdevils..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير ' 
msg.TheRank = 'مدير البوت ' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(vipdevils..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن '
msg.TheRank = 'ادمن في البوت '
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(vipdevils..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(vipdevils..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(vipdevils..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(vipdevils..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(vipdevils..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(vipdevils..':WELCOME_BOT'),[[•~مـرحبآ آنآ بوت آسـمـي ]]..redis:get(vipdevils..':NameBot:')..[[•~آختصـآصـي حمـآيه‌‏ آلمـجمـوعآت
...
if msg.GroupActive and redis:get(vipdevils..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(vipdevils..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(vipdevils..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(vipdevils..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.vipdevils then 
if msg.text and ScriptFile.ivipdevils then
for k, vipdevils in pairs(ScriptFile.vipdevils) do
local SearchText = msg.text:match(vipdevils)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.ivipdevils(msg,{msg.text:match(vipdevils)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",vipdevils.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dvipdevils then
if ScriptFile.dvipdevils(msg) == false then
return false
end
...
if Plug.vipdevils then 
if msg.text and Plug.ivipdevils then
for k, vipdevils in pairs(Plug.vipdevils) do
local SearchText = msg.text:match(vipdevils)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(vipdevils..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(vipdevils..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.ivipdevils(msg,{msg.text:match(vipdevils)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",vipdevils.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dvipdevils then
Plug.dvipdevils(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source vipdevils")
end 

end
else
print("The File Script.lua Not Runing in The Source vipdevils")
...
function tdcli_update_callback(data)
	...
	redis:srem(vipdevils..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/AhmedElsayed111/vipdevils/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/AhmedElsayed111/vipdevils/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/AhmedElsayed111/vipdevils/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/AhmedElsayed111/vipdevils/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/AhmedElsayed111/vipdevils/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/AhmedElsayed111/vipdevils/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(vipdevils..":SUDO_ID:")
redis:hset(vipdevils..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(vipdevils..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(vipdevils..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(vipdevils..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(vipdevils..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(vipdevils..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(vipdevils..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(vipdevils..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(vipdevils..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(vipdevils..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(vipdevils..'group:add'..data.chat_.id_) then
	redis:set(vipdevils..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(vipdevils..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(vipdevils..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(vipdevils..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	vipdevils..'group:name'..k,IDS.Title,
	vipdevils..'num_msg_vipdevils'..k,5,
	vipdevils..'group:add'..k,true,
	vipdevils..'lock_link'..k,true,
	vipdevils..'lock_spam'..k,true,
	vipdevils..'lock_webpage'..k,true,
	vipdevils..'lock_markdown'..k,true,
	vipdevils..'lock_flood'..k,true,
	vipdevils..'lock_bots'..k,true,
	vipdevils..'mute_forward'..k,true,
	vipdevils..'mute_contact'..k,true,
	vipdevils..'mute_document'..k,true,
	vipdevils..'mute_inline'..k,true,
	vipdevils..'lock_username'..k,true,
	vipdevils..'replay'..k,true
	)
	redis:sadd(vipdevils..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(vipdevils..'username:'..ID,'username',user)
	redis:sadd(vipdevils..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(vipdevils..'username:'..ID,'username',user)
	redis:sadd(vipdevils..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(vipdevils..'username:'..ID,'username',user)
	redis:sadd(vipdevils..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'•~** تم رفع آلنسـخه‏‏ آلآحتيآطـيه\n•~** حآليآ عدد مـجمـوعآتك هيه *'..redis:scard(vipdevils..'group:ids')..'* ®️\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(vipdevils..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(vipdevils..'username:'..data.user_.id_).username
	...
	redis:hset(vipdevils..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
